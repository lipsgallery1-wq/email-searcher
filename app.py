...
class EmailSearcher:
        ...
        for link in soup.find_all('a', href=True):
            ...
            if not keyword:
                ...
                if parsed.scheme in ['http', 'https'] and parsed.netloc:
                    if not any(href.lower().endswith(ext) for ext in config.IGNORED_EXTENSIONS):
                        links.add(full_url)
    ...
    def _extract_urls_from_json(self, data, base_url, links):
        ...
        if isinstance(data, dict):
            ...
            parsed = urlparse(full_url)
            if parsed.scheme in ['http', 'https'] and parsed.netloc:
                if not any(data.lower().endswith(ext) for ext in config.IGNORED_EXTENSIONS):
                    links.add(full_url)
    ...
    def search_emails(self, url, max_pages=1000, max_workers=8, known_emails=None, excluded_links=None, keyword='informations'):
        """Rechercher les emails sur un site web (avec système anti-doublons optimisé)"""
        logger.info(f"🚀 DÉBUT recherche d'emails - URL: {url}, max_pages: {max_pages}, mot-clé: '{keyword if keyword else 'TOUS'}'")
